# Developer Guide: Sovereign Diagnostic Tree
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/diagnostic-tree.git
cd diagnostic-tree
...
```bash
export SBB_FAULT_TREE_PATH=your_value_here
export SBB_ML_MODEL_PATH=your_value_here
export SBB_CONFIDENCE_THRESHOLD=your_value_here
export SBB_DOMAIN=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-diagnostic-tree)
...
```bash
# Build
docker build -t sovereign-diagnostic-tree:dev .

# Run with env file
docker run --env-file .env -p 8791:8791 sovereign-diagnostic-tree:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-diagnostic-tree
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8791`
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-diagnostic-tree",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8791` in use | `lsof -i :8791` | Kill PID or change `SERVICE_PORT` |
